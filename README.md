# Hey, I'm Shamil 

### Information Systems Undergraduate · Aspiring Business Analyst

I'm a final-year **Information Systems undergraduate at the University of Colombo School of Computing (UCSC)** with an interest in **Business Analysis, Project Management, and Technology**.

I enjoy working at the intersection of **business and technology** — understanding problems, collaborating with stakeholders, and turning requirements into practical digital solutions.

---

### 🛠️ Tech Stack

**Languages**
`JavaScript` `PHP` `C` `C++`

**Web & Frameworks**
`HTML` `CSS` `React` `Next.js` `Laravel`

**Backend & Data**
`REST APIs` `MySQL` `Sanity`

**Tools**
`Git` `GitHub` `Jira` `Confluence` `Power BI` `Vercel`

---

### 🚀 Projects

**FitVerse 2.0** — Gym Management System
PHP · MySQL · JavaScript

**Winter Shop** — E-commerce Platform
Laravel · PHP · MySQL · PayHere

**Nurture Nest** — Client NGO Website
Next.js · Sanity · Resend · Vercel

**Muslim Majlis** — Web Platform
Next.js · Tailwind CSS · Sanity

> Some projects are private/client projects and therefore aren't publicly available.

---

### 👨‍💼 Leadership

**President — Rotaract Club of UCSC**
*RI Year 2026–2027*

*Empowering Future Leaders. Elevating Communities. Evolving for Lasting Impact.*

---

### 🎯 Currently

* 📋 Developing my **Business Analysis & Project Management** skills
* ⚡ Exploring **React & Next.js**
* 📊 Building knowledge in **Power BI & data-driven decision making**
* 🤝 Developing **stakeholder management and leadership** skills

---

### 📫 Let's Connect

**LinkedIn:** [linkedin.com/in/shamil-shiraj](https://www.linkedin.com/in/shamil-shiraj)
**Email:** [shamilshiraj01@gmail.com](mailto:shamilshiraj01@gmail.com)

<p align="center">
  <i>Business understanding · Technical thinking · Building with purpose</i>
</p>
